...
recs <- readRDS("data/records_extra.rds")
...
dir.create("data/chunks_extra", showWarnings = FALSE, recursive = TRUE)
chunks <- split(recs$pmcid, ceiling(seq_len(nrow(recs)) / 100))
for (k in names(chunks)) {
  cf <- file.path("data/chunks_extra", paste0("flags_", k, ".rds"))
...
flags <- bind_rows(lapply(list.files("data/chunks_extra", full.names = TRUE), readRDS))
message("processed: ", nrow(flags), "  open data: ", sum(flags$is_open_data, na.rm = TRUE))
saveRDS(flags, "data/flags_extra.rds")
